"""AURORA Lightning AI Worker Client.

Connects a Lightning AI GPU instance to AURORA CORE's Compute Fabric.
Implements Worker Protocol v2: REGISTER, HEARTBEAT, CAPABILITIES, JOB lifecycle.
Includes full Model Runtime support: load, unload, infer, discover, health.
...
class AuroraLightningWorker:
    ...
    Includes full Model Runtime support for real GPU inference.
    """
    ...
    def worker_id(self) -> str:
        if self._worker_id is None:
            ...
            self._worker_id = f"lightning-{hostname}-{int(self._start_time)}"
    ...
    def _handle_job(self, job: dict) -> None:
        """Legacy job handler for backward compatibility."""
        job_id = job.get("job_id")
        workload_type = job.get("workload_type", "")
        payload = job.get("payload", {})
        self._current_job_id = job_id
    ...
    def _execute_workload(self, workload_type: str, payload: dict) -> dict:
        if workload_type == "BENCHMARK":
            ...
            return {"provider": "lightning", "workload": "INFERENCE", "message": "GPU inference ready"}
        elif workload_type == "EMBEDDINGS":
            return {"provider": "lightning", "workload": "EMBEDDINGS", "message": "GPU embeddings ready"}
